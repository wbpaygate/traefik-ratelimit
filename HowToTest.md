...
Тестирование локально и в gitlab CI реализовано через `docker compose`
Локально достаточно вызвать команду `task` или `task local_test`
В CI автоматически вызываются команды:
```
...
### script/test.sh
Этот скрипт написан для тестирования основной логики плагина: отправляются N+1 запросов, где N, это установленный limit, и последний запрос должен получить код 427(TOO MANY REQUESTS) 
