          ...
          - паттерн может содержать в элементе пути ```*``` это означает эта часть пути запроса может содержать любое значение, но эта часть пути должна присутствовать обязательно.
            например:
              - паттерн ```"/api/v2/*/*/methods"``` будет соответствовать пути запроса начинающегося с /api/v2 далее два следующие элемента должны присутствовать, но их значение может быть любым включая пустое
                ...
                но не будет соответствовать путь ```"/api/v2/work/space/methods_type1"```
          - патерн может содержать в конце символ ```$```
...
-  примеры правил:
   ...
   - ```
     ...
     т.к. во втором лимите присуствует идентичное правило, то оно будет проигнорировано и лимит 10000 будет применен только к правилу
